...
class SaleTeam(models.Model):
    _name = "custody.clear.team.custom"
    _description = "Custody Clear Team"
    ...
    approver_ids = fields.One2many('custody.clear.team.approver','team_custom_id', string='Approvers')

    @api.constrains('company_id')
    def _check_team_company(self):
        for team in self:
            if team.company_id.custody_clear_approval_route == 'no':
...
class SaleTeamApprover(models.Model):
    _name = "custody.clear.team.approver"
    _description = "Custody Clear Team Approver"
    _order = 'sequence'
    _rec_name = 'sequence'

    sequence = fields.Integer(string='Sequence', default=10)

    team_custom_id = fields.Many2one('custody.clear.team.custom', string='Team', ondelete='cascade')
...
class CustodyApprover(models.Model):
    _name = "custody.clear.order.approver"
    _inherit = "custody.clear.team.approver"
    _description = "Custody clear Approver"
    _order = 'state asc'

    team_approver_id = fields.Many2one(
        comodel_name='custody.clear.team.approver', string='Custody Team Approver',
        ondelete='set null')

    order_id = fields.Many2one(
        comodel_name='custody.clear.request', string='Order',
