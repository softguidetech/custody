...
class CustodyRequest(models.Model):
    _inherit = "custody.clear.request"

    custody_approval_route = fields.Selection(related='company_id.custody_clear_approval_route',
    ...
    team_custom_id = fields.Many2one(
        comodel_name="custody.clear.team.custom",required=True, string="Custody Team", domain="[('company_id', '=', company_id)]",
        readonly=True, states={'draft': [('readonly', False)], 'sent': [('readonly', False)]}, ondelete="restrict"
    )

    approver_ids = fields.One2many(
        comodel_name="custody.clear.order.approver", inverse_name="order_id", string="Approvers", readonly=True)

    current_approver = fields.Many2one(
        comodel_name="custody.clear.order.approver", string="Approver",
        compute="_compute_approver", store=True, compute_sudo=True)

    next_approver = fields.Many2one(
        comodel_name="custody.clear.order.approver", string="Next Approver",
        compute="_compute_approver", store=True, compute_sudo=True)
                ...
                if order.current_approver.user_id == self.env.user or self.env.is_superuser():
                    ...
                    if order.next_approver:
                        ...
                        order.message_post_with_view(
                            'custody_clear_request.order_approval',
                            subject=_('Cash Reconcile Approved: %s') % (order.name,),
    ...
    def button_reject(self):
        for order in self:
            partner = order.user_id.partner_id if order.user_id else order.create_uid.partner_id
            order.message_post_with_view(
                'custody_clear_request.order_reject',
    ...
    def generate_approval_route(self):
        ...
        for order in self:
            ...
            for team_approver in order.team_custom_id.approver_ids:
                ...
                # raise ('jjjjjjjjjjj')
                self.env['custody.clear.order.approver'].create({
    ...
    def send_to_approve(self):
        for order in self:
            ...
            order.with_user(order.user_id).message_post_with_view(
                'custody_clear_request.request_to_approve_so',
                subject=_('Cash Reconcile Approval: %s') % (order.name,),
                composition_mode='mass_mail',
